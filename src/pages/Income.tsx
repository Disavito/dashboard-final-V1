...
function Income() {
  ...
  const columnsWithActions: ColumnDef<IngresoType>[] = useMemo( // Use useMemo for columns
    ...
    [] // No dependencies needed if incomeColumns is static, or add incomeColumns if it changes
  );
  ...
  const customGlobalFilterFn = useCallback((row: Row<IngresoType>, _columnId: string, filterValue: any) => {
    ...
    const numeroOperacion = income.numeroOperacion?.toLowerCase() || '';
  ...
  if (loading || accountsLoading) {
    return (
      <div className="min-h-screen bg-background text-text font-sans flex items-center justify-center">
        ...
        <p className="ml-4 text-lg">Cargando ingresos y cuentas...</p>
      </div>
    );
  }
